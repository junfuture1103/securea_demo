...
let SignalingPort = parseInt(parsed.localPort)
...
const socket = io("https://" + remote_ip + ":" + SignalingPort + "/mediaserver", {
...
socket.on('transport-produce-data', async ({sctpStreamParameters, label, protocol, socketId}, callback) => {
    console.log('transport-dataproducer called')
...
socket.on('transport-produce', async ({socketId, kind, rtpParameters, appData}, callback) => {
    console.log('transport-producer called')
...
socket.on('createPipeTransport', async (myRoomNumber, oppositeRoomNumber) => {
    
    let pipeTransport = getPipeTransport(myRoomNumber);

    if (pipeTransport === undefined) {
        pipeTransport = router.createPipeTransport({
            listenIp : local_ip
        })
    ...
    socket.emit('connectPipeTransport', {
        myRoomNumber,
        oppositeRoomNumber,
  ...
  socket.on('connectPipeTransport', async ({myRoomNumber, oppositeRoomNumber, ip, port, srtpParameters}) => {
    ...
    socket.emit('informAllProducers', {
        myRoomNumber, oppositeRoomNumber,
        producers,
    })


    socket.on('informAllProducers', ({
        myRoomNumber,
        oppositeRoomNumber,
        producers
    }) => {
