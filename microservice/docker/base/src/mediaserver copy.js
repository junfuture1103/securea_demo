...
const socket = io("https://192.168.1.231:443/mediaserver", {
...
const addDirectConsumer = async (producer) => {
    ...
    directConsumer.on('message', async function(message, ppid){
      ...
      for (let i in directProducers) {
        //console.log(`directProducersId : ${directProducers[i].clientId} and to :${parsed_msg.clientId}`)
        if(directProducers[i].clientId !== parsed_msg.clientId){
          directProducers[i].producer.send(
            JSON.stringify({
              clientId : parsed_msg.clientId,
              type : "command",
