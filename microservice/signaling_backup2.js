...
connections_client.on('connection', async socket => {
    ...
    socket.on('pipetoRoom', ({myRoomNumber, oppositeRoomNumber}) => {
        if(mediaservers[oppositeRoomNumber] !== undefined) {
            mediaservers[oppositeRoomNumber].emit('createPipeTransport', {myRoomNumber});
        } else {
...
connections_mediaserver.on('connection', async socket => {
    ...
    socket.on('informAllProducers', ({myRoomNumber, oppositeRoomNumber, producers}) => {
        mediaservers[oppositeRoomNumber].socket.emit('informAllProducers', {
            myRoomNumber,
            oppositeRoomNumber,
            producers
        })
