...
export default class RoomClient {
	...
	constructor(
		...
		logger.debug(
			'CONSTRUCTURE() [roomId:"%s", peerId:"%s", displayName:"%s", device:%s]',
				...
				case 'newDataConsumer':
					{
						...
						try {
							...
							dataConsumer.on('message', (message) => {
								...
								console.log("welcome to secureA API server! msg : ", message)
								console.log("welcome to secureA API server! msg first latter : ", message[0])

								if (message[0] == '{') {
									this.requestProcess(message)
								}
	...
	async requestProcess(requestmsg) {
		logger.debug('requestProcess() [requestmsg:"%s]', requestmsg);
		const respondMsg = requestapi.processRequest(requestmsg);

		try {
			this.sendChatMessage(respondMsg)
		}
		catch (error) {
			logger.error('respond to client failed:%o', error);
		}
	}
